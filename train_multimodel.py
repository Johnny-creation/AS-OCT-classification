...
def train_model(model_names, batch_size=32, epochs=5, learning_rate=0.001, num_workers=4):
    ...
    for model_name in model_names:
        ...
        # 根据模型选择输入大小
        model, input_size = get_model(model_name, num_classes)
        ...
        for epoch in range(num_epochs):
            ...
            for phase in ['train', 'val']:
                ...
                if phase == 'val' and epoch_acc > best_acc:
                    ...
                    print(f'新的最佳模型已保存至 {model_save_path}，准确率: {best_acc:.4f}')
            
            print()
        
        print(f'模型 {model_name} 训练完成。最佳验证准确率: {best_acc:.4f}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default=[
                                'convnext_tiny', 'swin_t', 'mobilenet_v2'], 
                        nargs='+',  # 允许接收一个或多个值
                        choices=['resnet34', 'densenet169', 'efficientnet_b3', 'vgg16', 'vit', 
                             'convnext_tiny', 'swin_t', 'mobilenet_v2'],
                        help='选择要训练的模型（可多选）')
